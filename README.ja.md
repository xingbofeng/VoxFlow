<div align="center">
  ...
  <h1>VoxFlow</h1>
  <p><strong>音声、スクリーンショット、クリップボード履歴、coding-agent 指示のための macOS アセットワークベンチ。</strong></p>
  <p><code>⌥Space</code> でランチャーを開き、最近の音声、スクリーンショット、クリップボード資産をすぐ呼び戻せます。音声入力、キャプチャ、コピーした内容は、検索できて、コピーできて、再利用できるローカル履歴になります。</p>
...
VoxFlow は、今使っているアプリに寄り添うアセットワークベンチ兼クイックランチャーです。音声アシスタントではありません。ウィンドウを奪わず、メッセージを自動送信せず、別の入力欄へ移動させません。音声、スクリーンショット、クリップボード項目、Agent コマンドを、検索・プレビュー・再利用できるローカル資産にして、現在の作業場所へ戻します。
...
| スクリーンショットを処理 | `⌘⇧A` を押して範囲選択 | OCR 結果パネル | 翻訳、要約、読み上げは任意 |
...
| スクリーンショット記録を見る | Workbench -> Screenshot | ローカルのスクリーンショット履歴と OCR テキスト | ローカル保存。検索、お気に入り、コピー、削除に対応 |
...
| Page | できること |
| --- | --- |
| Home | 資産履歴、今日の追加、ソース内訳、再利用可能な内容を確認。音声、スクリーンショット、クリップボード資産を検索、コピー、削除 |
...
| Screenshot | OCR テキスト付きスクリーンショットを閲覧。お気に入り、検索、ページング対応 |
...
- **Asset history workbench**: ASR 成功テキスト、スクリーンショット、クリップボードのテキスト/画像/ファイル/リンク/色が 1 つの資産システムに入ります。Home では資産数、ソース内訳、再利用可能な内容を確認できます。
...
- **Screenshot record library**: キャプチャしたスクリーンショットは Screenshot ページに保存され、OCR テキスト、お気に入り、検索、ワンクリックコピー/削除に対応します。
...
| Screen Recording | Agent Compose と screenshot OCR のために現在ウィンドウを OCR する | System Settings -> Privacy & Security -> Screen Recording |
...
- Screenshot records（`⌘⇧A` で取得した OCR text + screenshot files）はローカル保存され、アップロードされません。
...
| スクリーンショット記録が見つからない | Settings -> Data & Privacy -> Data Management で storage health を確認し、data folder を開いて `Application Support/VoxFlow/Screenshots/` に画像記録があるか確認してください。Screen Recording 権限も確認してください |
...
VoxFlow は GPLv3 で配布されます。サードパーティ components はそれぞれの license notice と attribution を保持します。`docs/third-party-licenses.md` を参照してください。
