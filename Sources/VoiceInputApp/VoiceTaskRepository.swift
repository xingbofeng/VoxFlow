...
final class VoiceTaskRepository {
    ...
    private func bind(_ task: VoiceTask, to statement: SQLiteStatement) throws {
        ...
        try statement.bind(String(data: warningsData, encoding: .utf8) ?? "[]", at: 16)
        try statement.bind(task.trace, at: 17)
        try statement.bind(formatter.string(from: task.createdAt), at: 18)
        try statement.bind(formatter.string(from: task.updatedAt), at: 19)
        try statement.bind(task.completedAt.map(formatter.string(from:)), at: 20)
    }
    ...
    private func row(from statement: SQLiteStatement) throws -> VoiceTask {
        guard let id = statement.columnString(at: 0),
              ...
              let createdAtText = statement.columnString(at: 17),
              let updatedAtText = statement.columnString(at: 18),
        ...
        let warningsJson = statement.columnString(at: 15) ?? "[]"
        let warnings = (try? JSONDecoder().decode(
            [String].self,
            from: Data(warningsJson.utf8)
        )) ?? []

        let completedAt = statement.columnString(at: 19).flatMap(formatter.date(from:))
        ...
        return VoiceTask(
            ...
            warnings: warnings,
            trace: statement.columnString(at: 16),
            createdAt: createdAt,
            updatedAt: updatedAt,
