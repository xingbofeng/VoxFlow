...
final class SettingsWindowController: NSWindowController {
    ...
    @objc private func downloadQwen3Model(_ sender: NSButton) {
        ...
        setASRStatus("准备下载 \(asrManager.qwen3ModelSize.rawValue) 模型...", color: .secondaryLabelColor)

        let downloader = Qwen3ModelDownloader()
        modelDownloadTask = Task { [weak self] in
            do {
                let modelURL = try await downloader.download(manifest: manifest) { [weak self] progress in
                    self?.updateModelDownloadProgress(progress)
                }
                await MainActor.run { [weak self] in
                    guard let self else { return }
                    self.asrManager.qwen3ModelPath = modelURL.path
