...
final class FunASRASRSession: VoxFlowASRCore.ASRSession, @unchecked Sendable {
    ...
    private var transcriberTask: Task<any FunASRTranscribing, Error>?
    ...
    init(
        ...
        modelURL: URL,
        transcriberFactory: any FunASRTranscriberMaking
    ) {
    ...
    func finish() async throws {
        ...
        do {
            guard Self.containsAudibleSamples(samples) else {
                ...
                throw FunASRProviderError.emptyTranscript
            }
            ...
            guard !text.isEmpty else {
                emitFailure(
                    VoxFlowASRCore.ASRError(
                        ...
                        message: FunASRProviderError.emptyTranscript.localizedDescription
                    )
                )
                throw FunASRProviderError.emptyTranscript
            }
        ...
        eventStream.yield(
            .failure(
                ...
                error: VoxFlowASRCore.ASRError(
                    category: .cancelled,
                    message: "FunASR session was cancelled."
                )
            )
        )
        eventStream.finish()
    ...
    private func emitFailure(_ error: Error) {
        emitFailure(FunASRASRProvider.asrError(for: error))
    }
