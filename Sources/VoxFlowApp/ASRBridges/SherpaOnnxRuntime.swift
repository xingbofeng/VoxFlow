import Foundation
import CSherpaOnnx

enum SherpaOnnxRuntimeError: LocalizedError {
    case modelFilesMissing
    case recognizerCreationFailed
    case transcriptionFailed

    var errorDescription: String? {
        switch self {
        case .modelFilesMissing:
            return "模型文件不完整，请重新下载。"
        case .recognizerCreationFailed:
            return "模型加载失败，请确认模型版本与运行时兼容。"
        case .transcriptionFailed:
            return "本地语音识别失败。"
        }
    }
}

enum SherpaOnnxRuntime {
    static var version: String {
        String(cString: SherpaOnnxGetVersionStr())
    }
}

final class SherpaOnnxRecognizer: @unchecked Sendable {
    private let recognizer: OpaquePointer

    init(variant: SherpaASRModelVariant, directoryURL: URL? = nil) throws {
        let root = directoryURL ?? variant.defaultDirectoryURL
        guard variant.modelsExist(at: root) else {
            throw SherpaOnnxRuntimeError.modelFilesMissing
        }

        let strings = [
            variant.modelPath.map { root.appendingPathComponent($0).path } ?? "",
            variant.encoderPath.map { root.appendingPathComponent($0).path } ?? "",
            variant.decoderPath.map { root.appendingPathComponent($0).path } ?? "",
            variant.tokensPath.map { root.appendingPathComponent($0).path } ?? "",
            variant.embeddingPath.map { root.appendingPathComponent($0).path } ?? "",
            variant.tokenizerPath.map { root.appendingPathComponent($0).path } ?? "",
            "",
        ]
        let pointers = strings.map { strdup($0) }
        defer { pointers.forEach { free($0) } }

        let type: VoxSherpaModelType = switch variant.family {
        case .funASR:
            VOX_SHERPA_FUNASR_NANO
        }
        var config = VoxSherpaModelConfig(
            type: type,
            model: UnsafePointer(pointers[0]),
            encoder: UnsafePointer(pointers[1]),
            decoder: UnsafePointer(pointers[2]),
            tokens: UnsafePointer(pointers[3]),
            embedding: UnsafePointer(pointers[4]),
            tokenizer: UnsafePointer(pointers[5]),
            language: UnsafePointer(pointers[6]),
            num_threads: 2
        )
        guard let recognizer = VoxSherpaCreateRecognizer(&config) else {
            throw SherpaOnnxRuntimeError.recognizerCreationFailed
        }
        self.recognizer = recognizer
    }

    deinit {
        VoxSherpaDestroyRecognizer(recognizer)
    }

    func transcribe(samples: [Float], sampleRate: Int32 = 16_000) throws -> String {
        guard !samples.isEmpty else { return "" }
        guard let result = samples.withUnsafeBufferPointer({
            VoxSherpaTranscribe(recognizer, $0.baseAddress, Int32($0.count), sampleRate)
        }) else {
            throw SherpaOnnxRuntimeError.transcriptionFailed
        }
        defer { VoxSherpaFreeText(result) }
        return String(cString: result)
    }
}
