<div align="center">
  ...
  <h1>碼上寫 · VoxFlow</h1>
  <p><strong>語音、截圖、剪貼簿和 coding-agent 指令的 macOS 資產工作台。</strong></p>
  <p>按 <code>⌥Space</code> 打開啟動台，找回最近語音、截圖和剪貼簿資產；按住說話、框選截圖、複製內容都會沈澱為可搜尋、可複製、可復用的歷史資產。</p>
...
碼上寫是一個貼在當前應用上的資產工作台和快速啟動台。它不是語音助手，不接管視窗，不自動傳送內容；它把語音、截圖、剪貼簿和 Agent 指令沈澱為可檢索、可預覽、可復用的本地資產，再送回你正在工作的地方。
...
| 截圖記錄回看 | 工作台 → 截圖 | 本地截圖歷史與 OCR 文本 | 本地保存，可搜尋、收藏、分頁、複製與刪除 |
...
| 頁面 | 可以做什麼 |
| --- | --- |
| 首頁 | 查看歷史資產、今日新增、來源分布和可復用內容；搜尋、複製或刪除語音、截圖和剪貼簿資產 |
...
| 截圖 | 瀏覽截圖記錄，查看原圖和 OCR 文本，支持收藏、搜尋、分頁和快捷複製/刪除 |
...
- **歷史資產工作台**：語音 ASR 成功結果、截圖、剪貼簿文本/圖片/文件/鏈接/顏色統一進入資產體系，首頁按資產數量、來源分布和可復用內容展示。
...
- **截圖記錄庫**：所有截圖記錄（原圖 + OCR 文本）都可回看，支持收藏、搜尋、分頁、複製與刪除。
...
| 屏幕錄制 | 為“任務助手”和截圖 OCR 讀取當前視窗文字與截圖；截圖記錄保存在本機便於回看 | 系統設置 -> 隱私與安全性 -> 屏幕錄制 |
...
每次 `⌘⇧A` 觸發的截圖都會被保留為本地截圖記錄，便於後續回看。你可以在工作台打開“截圖”頁面，使用關鍵詞搜尋、收藏篩選、分頁瀏覽，並複製 OCR 文本、複製原圖或刪除記錄。
...
- 截圖記錄（原圖 + OCR 文本）本地保存，用於後續回看，不會上傳到雲端。
...
| 截圖記錄找不到 | 去設置 → 數據與隱私 → 數據管理檢查存儲狀態；點擊“打開數據目錄”確認 `Application Support/VoxFlow/Screenshots/` 下是否有記錄文件；並確認已授權屏幕錄制 |
...
VoxFlow 以 GPLv3 分發。第三方組件仍保留各自許可證和歸屬說明，詳見 `docs/third-party-licenses.md`。
