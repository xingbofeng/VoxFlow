<div align="center">
  ...
  <img src="docs/assets/voiceinput-hero.svg" alt="码上写 - 语音输入、截图文字和 Agent 调度工作流" width="100%">

  <h1>码上写 · VoxFlow</h1>
  <p><strong>把口述、截图文字和 coding-agent 指令送回当前工作现场。</strong></p>
  <p>原生 macOS 菜单栏语音工作流入口：按住说话即可输入，框选截图即可 OCR，喊出任务助手即可投递给本地 Agent。</p>
...
码上写是一个贴在当前应用上的“语音键盘”和轻量工作流入口。它不是语音助手，不接管窗口，不自动发送内容；它只负责把声音、图片文字和 Agent 指令送回你正在工作的地方。
...
- 剪贴板图片仅用于当次 OCR 处理，不进入截图记录或历史持久化；区域截图（`Command + Shift + A`）会保存原图与 OCR 文本到本机以便回看。
