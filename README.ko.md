<div align="center">
  ...
  <h1>VoxFlow</h1>
  <p><strong>음성, 스크린샷, 클립보드 기록, coding-agent 지시를 위한 macOS 자산 워크벤치.</strong></p>
  <p><code>⌥Space</code>로 런처를 열고 최근 음성, 스크린샷, 클립보드 자산을 다시 찾을 수 있습니다. 받아쓰기, 캡처, 복사한 내용은 검색 가능하고 복사 가능하며 재사용 가능한 로컬 기록이 됩니다.</p>
...
VoxFlow는 지금 사용 중인 앱에 붙는 자산 워크벤치이자 빠른 런처입니다. 음성 비서가 아닙니다. 창을 빼앗지 않고, 메시지를 자동 전송하지 않으며, 다른 입력창으로 이동시키지 않습니다. 음성, 스크린샷, 클립보드 항목, Agent 명령을 검색, 미리보기, 재사용 가능한 로컬 자산으로 만들고 다시 현재 작업 공간으로 돌려보냅니다.
...
| 스크린샷 캡처와 처리 | `⌘⇧A`를 누르고 영역 선택 | OCR 결과 패널 | 번역, 요약, 읽어주기는 선택 사항 |
...
| 스크린샷 기록 보기 | Workbench -> Screenshot | 로컬 스크린샷 기록과 OCR 텍스트 | 로컬 저장. 검색, 즐겨찾기, 복사, 삭제 가능 |
...
| Page | 할 수 있는 일 |
| --- | --- |
| Home | 자산 기록, 오늘 추가된 항목, source breakdown, 재사용 가능한 내용을 확인하고 음성, 스크린샷, 클립보드 자산을 검색, 복사, 삭제 |
...
| Screenshot | OCR 텍스트가 포함된 캡처 스크린샷을 favorites, search, paging과 함께 탐색 |
...
- **Asset history workbench**: 성공한 ASR 텍스트, 스크린샷, 클립보드 텍스트/이미지/파일/링크/색상이 하나의 자산 시스템을 공유합니다. Home은 자산 수, source breakdown, 재사용 가능한 내용을 보여줍니다.
...
- **Screenshot record library**: 캡처한 스크린샷은 Screenshot 페이지에 OCR text, favorites, search, one-click copy/delete actions와 함께 보관됩니다.
...
1. `VoxFlow-1.8.2-macOS.dmg` 열기
2. `VoxFlow`를 `Applications` 폴더로 드래그
3. 첫 실행 시 macOS가 앱을 확인할 수 없다고 하면, Control-click 후 **Open** 선택

설치 후 Workbench -> Screenshot을 열어 스크린샷 기록과 OCR 기록이 사용 가능한지 확인할 수 있습니다.
...
| Screen Recording | Agent Compose와 screenshot OCR을 위해 현재 창 OCR | System Settings -> Privacy & Security -> Screen Recording |
...
- Screenshot records(`⌘⇧A`로 캡처한 OCR text + screenshot files)는 로컬에 저장되며 업로드되지 않습니다.
...
| 스크린샷 기록이 없음 | Settings -> Data & Privacy -> Data Management에서 storage health를 확인하고 data folder를 열어 `Application Support/VoxFlow/Screenshots/`에 이미지 기록이 있는지 확인하세요. Screen Recording 권한도 확인하세요 |
...
VoxFlow는 GPLv3로 배포됩니다. third-party components는 각자의 license notices와 attribution을 유지합니다. `docs/third-party-licenses.md`를 참고하세요.
