...
VoxFlow는 지금 사용 중인 앱 안에 머무르는 음성 입력 도구입니다. 음성 비서가 아닙니다. 창을 가져가거나, 다른 입력창으로 이동시키거나, Enter를 누르거나, 메시지를 자동 전송하지 않습니다.

먼저 음성 키보드이며, 작업 중 생기는 음성, 스크린샷, 화면 녹화, 클립보드, 노트, 로컬 coding-agent 지시를 검색 가능한 로컬 자산으로 정리합니다.
