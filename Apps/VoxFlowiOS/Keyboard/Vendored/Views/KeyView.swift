...
// Vendored from giellakbd-ios Keyboard/Views/KeyView.swift
...
final class KeyView: UIView {
    ...
    var isSwipeKey: Bool {
        if case let .input(_, alt) = key.type, alt != nil {
            return alt.map(Self.isVisibleAlternate) ?? false
        }
        switch key.type {
        case .comma, .fullStop:
            return true
    ...
    private static func isVisibleAlternate(_ alternate: String) -> Bool {
        if alternate == "accent" { return false }
        if alternate == "switchChineseNineGrid" { return false }
        if alternate == "switchChineseQwerty" { return false }
        if alternate == "switchEnglish" { return false }
        if alternate == "switchNumbers" { return false }
        if alternate == "switchSymbols" { return false }
        if alternate == "openChineseSymbols" { return false }
        if alternate == "openChineseNumbers" { return false }
        if alternate == "clearChineseComposition" { return false }
        if alternate.hasPrefix("chineseNineGridDigit:") { return false }
        return true
    }
    ...
    private func input(string: String, alt: String?, page: KeyboardPage) {
        ...
        if let alternateString = alt, Self.isVisibleAlternate(alternateString) {
