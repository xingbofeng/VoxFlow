    ...
    private func makeProvider(
        ...
        case "qwen3":
            ...
            let variant: Qwen3ModelVariant = environment["VOICEINPUT_TEST_QWEN3_VARIANT"] == "1.7b"
                ? .qwen17MLX4Bit
                : .qwen06CoreMLInt8
            let sessionFactory: any Qwen3StreamingSessionMaking
            switch variant {
            ...
            case .qwen17MLX4Bit:
                let workerURL = try Self.repositoryRoot()
                    .appendingPathComponent(
                        "Sources/VoxFlowProviderQwen3/Workers/voxflow-qwen3-mlx-worker"
                    )
                let runtimeLayout = Qwen3MLXRuntimeLayout.current()
                sessionFactory = Qwen3MLXWorkerStreamingSessionFactory(
                    launchCommandProvider: {
                        Qwen3MLXWorkerExecutableResolver.LaunchCommand(
                            executableURL: runtimeLayout.pythonExecutableURL,
        ...
        case "paraformer":
            throw XCTSkip("Paraformer is listed in the UI as planned/unavailable and is not linked into this build.")
