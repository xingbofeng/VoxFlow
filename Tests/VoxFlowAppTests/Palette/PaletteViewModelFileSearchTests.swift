...
final class PaletteViewModelFileSearchTests: XCTestCase {
    ...
    func testNewQueryIgnoresOlderSearchResult() async throws {
        let searchService = QueryMappedPaletteFileSearchService(
            responses: [
                "1": PaletteFileSearchResponse(
                    query: "1",
                    items: [makeFileItem(name: "old.txt")],
                    completion: .completed
                ),
                "12": PaletteFileSearchResponse(
                    query: "12",
...
@MainActor
private final class QueryMappedPaletteFileSearchService: PaletteFileSearching {
    private let responses: [String: PaletteFileSearchResponse]
    private let delays: [String: UInt64]

    init(
        responses: [String: PaletteFileSearchResponse],
    ...
    func search(_ request: PaletteFileSearchRequest) async -> PaletteFileSearchResponse {
        let delay = delays[request.query] ?? 0
        if delay > 0 {
