...
class ReleaseMetadataTests(unittest.TestCase):
    ...
    def prepare(
        ...
        arguments = ("--version", self.version, "--build", self.build)
    ...
    def test_macos_and_windows_preparation_omits_ios_assets_and_syncs_sources(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertNotIn(ios_ipa, (fixture_root / "docs/index.html").read_text(encoding="utf-8"))
    ...
    def test_default_preparation_preserves_three_platform_ios_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            completed = self.prepare(fixture_root)
            ...
            self.assertEqual(ios["ipa"]["name"], f"Mashangxie-{self.version}-iOS.ipa")
            release_notes = (fixture_root / f".github/release-notes/v{self.version}.md").read_text(
                encoding="utf-8"
            )
