...
public sealed class WindowsLowLevelKeyboardHook : IKeyboardHookBackend, IKeyboardHookHealth
{
    ...
    private readonly Action<LowLevelKeyEvent> onKeyEvent;
    private readonly TimeProvider timeProvider;
    private readonly ManualResetEventSlim startupCompleted = new(false);
    private readonly HookProcedure hookProcedure;
    private readonly Channel<LowLevelKeyEvent> eventChannel =
        Channel.CreateUnbounded<LowLevelKeyEvent>(new UnboundedChannelOptions
        {
            SingleReader = true,
            SingleWriter = true,
    ...
    public WindowsLowLevelKeyboardHook(
        Action<LowLevelKeyEvent> onKeyEvent,
        TimeProvider? timeProvider = null)
    {
        this.onKeyEvent = onKeyEvent ?? throw new ArgumentNullException(nameof(onKeyEvent));
    ...
    public void Dispose()
    {
        ...
        try
        {
            _ = dispatchTask.Wait(TimeSpan.FromSeconds(3));
        }
        catch (AggregateException)
        {
            // Dispatch failures never escape disposal of the native hook.
        }
    ...
    private nint OnHook(int code, nint message, nint data)
    {
            ...
            && message.ToInt64() is WmKeyDown or WmKeyUp or WmSysKeyDown or WmSysKeyUp)
        {
            ...
            {
                eventChannel.Writer.TryWrite(new LowLevelKeyEvent(
                    native.VirtualKey,
                    native.ScanCode,
                    flags,
    ...
    {
        await foreach (var keyEvent in eventChannel.Reader.ReadAllAsync().ConfigureAwait(false))
        {
            try
            {
                onKeyEvent(keyEvent);
            ...
            {
                // Consumers report their own failures; the input pump remains available.
            }
        }
    }
