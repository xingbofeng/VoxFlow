        ...
        int limit)
    {
        ...
        return transactionRunner.Read(connection =>
        {
            ...
            command.CommandText =
                "SELECT id, source, raw_text, final_text, metadata_json, created_at_unix_ms " +
                "FROM dictation_history " +
                "WHERE (raw_text LIKE $query ESCAPE '\\' OR final_text LIKE $query ESCAPE '\\') " +
                "AND ($dateFrom IS NULL OR created_at_unix_ms >= $dateFrom) " +
                "AND ($dateTo IS NULL OR created_at_unix_ms <= $dateTo) " +
                "ORDER BY created_at_unix_ms DESC, id ASC LIMIT $limit;";
            command.Parameters.AddWithValue("$query", "%" + EscapeLike(query.Trim()) + "%");
            command.Parameters.AddWithValue("$dateFrom", dateFromUtc?.ToUnixTimeMilliseconds() is { } from ? from : DBNull.Value);
            command.Parameters.AddWithValue("$dateTo", dateToUtc?.ToUnixTimeMilliseconds() is { } to ? to : DBNull.Value);
            command.Parameters.AddWithValue("$limit", limit);
            using var reader = command.ExecuteReader();
            List<HistoryEntry> entries = [];
            while (reader.Read())
            {
                entries.Add(new HistoryEntry(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3),
                    DeserializeMetadata(reader.GetString(4)), DateTimeOffset.FromUnixTimeMilliseconds(reader.GetInt64(5))));
            }
            return entries;
        });
    }
