...
public sealed class OpenAiSettingsCardIntegrationTests
{
    ...
    public async Task Save_and_confirmed_delete_refresh_the_shared_state_while_cancelled_delete_preserves_it()
    {
        ...
        // OpenAI credentials live under Models → LLM, not Text (mac parity).
        Assert.True(settings.TryNavigate("text"));
        Assert.IsType<TextSettingsPageViewModel>(settings.CurrentPage);
        Assert.Null(typeof(TextSettingsPageViewModel).GetProperty("OpenAi"));

        Assert.True(settings.TryNavigate("models"));
        dynamic models = Assert.IsType<ModelsSettingsPageViewModel>(settings.CurrentPage);
        dynamic card = models.OpenAi;
