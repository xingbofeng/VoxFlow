...
public sealed partial class FileTranscriptionLocalizationTests
{
    ...
    [Fact]
    public void All_five_locales_have_the_same_file_transcription_keys_and_placeholders()
    {
        ...
        var resourcesByCulture = Cultures.ToDictionary(
            culture => culture,
            culture => ReadFileTranscriptionResources(manager, culture));
        var invariant = resourcesByCulture[CultureInfo.InvariantCulture];

        Assert.NotEmpty(invariant);
        foreach (var (culture, resources) in resourcesByCulture)
        {
    ...
    private static Dictionary<string, string> ReadFileTranscriptionResources(
        ...
        CultureInfo culture)
    {
        ...
        return set.Cast<DictionaryEntry>()
            .Where(entry => entry.Key is string key
                && key.StartsWith("FileTranscription", StringComparison.Ordinal))
            .ToDictionary(
