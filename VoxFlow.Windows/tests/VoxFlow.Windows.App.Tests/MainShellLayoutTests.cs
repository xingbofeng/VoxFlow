...
public sealed class MainShellLayoutTests
{
    ...
    public async Task Main_window_enforces_the_v1_minimum_size_contract()
    {
        await StaWpfTestHost.RunAsync(_ =>
        {
            ...
            Assert.Equal(1260D, window.MinWidth);
    ...
    [Fact]
    public void Navigation_exposes_file_transcription_between_home_and_settings()
    {
        var viewModel = new MainShellViewModel();

        Assert.Equal(
            [ShellRoute.Home, ShellRoute.FileTranscription, ShellRoute.Settings],
        ...
        Assert.False(viewModel.TryNavigate("screenshot"));
        Assert.Equal(ShellRoute.Home, viewModel.CurrentRoute);
    }

    [Fact]
    public void Shell_route_contract_contains_no_screenshot_destination()
    {
        Assert.Equal(
            [nameof(ShellRoute.Home), nameof(ShellRoute.FileTranscription), nameof(ShellRoute.Settings)],
