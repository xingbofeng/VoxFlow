...
{
    [Fact]
    public async Task Settings_exposes_only_general_models_voice_and_text_with_two_model_tabs()
    {
        ...
        Assert.Equal(new[] { "asr", "llm" }, ItemIds((IEnumerable)models.Tabs));
        Assert.True(models.TrySelectTab("llm"));
        Assert.False(models.TrySelectTab("downloads"));
        Assert.Equal("llm", (string)models.SelectedTabId);
    }
    ...
    public async Task Voice_page_has_five_cards_and_writes_shortcut_and_middle_mouse_to_shared_state()
    {
        ...
        Assert.Equal("right-control", (string)voice.ShortcutId);
    }
    ...
    [Fact]
    public async Task Models_and_Text_share_one_fixed_official_OpenAI_card_without_a_custom_endpoint()
    {
        var viewModel = Create(new VoxFlowStateStore(), new FakeTextSettingsStore());
        await InvokeAsync(viewModel, "InitializeAsync");
        Assert.True(((dynamic)viewModel).TryNavigate("models"));
        ...
        Assert.True(((dynamic)viewModel).TryNavigate("text"));
        dynamic textOpenAi = ((dynamic)viewModel).CurrentPage.OpenAi;

        Assert.Same((object)modelsOpenAi, (object)textOpenAi);
        Assert.Equal("https://api.openai.com/v1", (string)textOpenAi.BaseUrl);
        Assert.False((bool)textOpenAi.CanEditBaseUrl);
        Assert.DoesNotContain("tokenhub", (string)textOpenAi.BaseUrl, StringComparison.OrdinalIgnoreCase);
        Assert.Equal(
            new[] { "save", "test", "delete" },
            ItemIds((IEnumerable)textOpenAi.Actions));
    }
