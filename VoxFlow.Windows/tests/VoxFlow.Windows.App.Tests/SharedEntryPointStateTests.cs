...
public sealed class SharedEntryPointStateTests
{
    ...
    [Fact]
    public void Agent_readiness_and_running_phase_are_projected_from_the_same_snapshot()
    {
        var store = new VoxFlowStateStore();
        dynamic projection = Create(
            "VoxFlow.Windows.App.State.SharedAppStateProjection",
        ...
        coordinator.RecordAgentReadiness(
            runtimeAvailable: true,
            providerConfigured: true,
            connectionHealthy: true,
            toolCallingSupported: true);
        coordinator.RecordAgentPhase(DictationPhase.Processing);

        var tray = ((ITrayMenuStateSource)projection).Read();
        Assert.True(tray.Agent.IsReady);
        Assert.True(tray.Agent.IsRunning);
        Assert.Equal(DictationPhase.Processing, tray.Agent.Phase);
        Assert.Equal("agent.status.running", tray.Agent.StatusId);
    }
