...
platform_project="${windows_root}/src/VoxFlow.Windows.Platform/VoxFlow.Windows.Platform.csproj"
...
grep -Eq '<PackageReference Include="ScreenCapture\.NET\.DX11" Version="\[3\.0\.0\]"[[:space:]]*/>' "${platform_project}" || {
  printf '%s\n' 'Platform must directly pin ScreenCapture.NET.DX11 to the exact audited [3.0.0] range.' >&2
  exit 1
}
...
for inventory_entry in \
  'ScreenCapture.NET | 3.0.0 | LGPL-2.1-only' \
  'ScreenCapture.NET.DX11 | 3.0.0 | LGPL-2.1-only' \
  'HPPH | 1.0.0 | LGPL-2.1-only' \
  'Vortice.Direct3D11 | 3.5.0 | MIT' \
  'Vortice.DXGI | 3.5.0 | MIT' \
  'SharpGen.Runtime | 2.1.2-beta | MIT'; do
  grep -Fq "${inventory_entry}" "${notices}" || {
    printf 'Screenshot dependency inventory is missing: %s\n' "${inventory_entry}" >&2
    exit 1
  }
done

test -f "${windows_root}/installer/licenses/LICENSE-LGPL-2.1-only.txt" || {
  printf '%s\n' 'The LGPL-2.1 license text for replaceable screenshot DLLs is missing.' >&2
  exit 1
}
