...
public sealed class TextOutputCoordinatorTests
{
    ...
    [Fact]
    public async Task Settings_aware_injector_switches_output_mode_without_recreation()
    {
        var mode = "quickPaste";
        var quickPaste = new StubTextOutputInjector(new OutputResult(OutputResultKind.Inserted));
        var simulatedTyping = new StubTextOutputInjector(new OutputResult(OutputResultKind.Inserted));
        var injector = new SettingsAwareTextOutputInjector(
            () => mode,
            quickPaste,
            simulatedTyping);

        _ = await injector.InjectAsync("first", CancellationToken.None);
        mode = "simulatedTyping";
        _ = await injector.InjectAsync("second", CancellationToken.None);

        Assert.Equal(1, quickPaste.CallCount);
        Assert.Equal(1, simulatedTyping.CallCount);
    }
