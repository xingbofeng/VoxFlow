...
public sealed class OpenAiSettingsTests
{
    ...
    [Fact]
    public async Task Production_card_has_a_fixed_official_base_URL_and_no_compatibility_defaults()
    {
        ...
        Assert.Equal(new Uri("https://api.openai.com/v1"), status.BaseUri);
        Assert.Equal("api.openai.com", status.BaseUri.Host);
