...
public sealed class HistoryRepositoryTests
{
    ...
    [Fact]
    public void Search_is_parameterized_escapes_like_wildcards_and_applies_date_limit()
    {
        using var fixture = new HistoryFixture(Now);
        fixture.Seed(CreateEntry("literal-100%_match", Now));
        fixture.Seed(CreateEntry("older-100%_match", Now.AddDays(-2)));
        fixture.Seed(CreateEntry("unrelated", Now));

        var results = fixture.Store.Search("100%_match", Now.AddDays(-1), null, 1);
        var injection = fixture.Store.Search("'); DROP TABLE dictation_history; --", null, null, 10);

        Assert.Equal(["literal-100%_match"], results.Select(entry => entry.Id));
        Assert.Empty(injection);
        Assert.Equal(3, fixture.Store.ReadAll().Count);
    }
