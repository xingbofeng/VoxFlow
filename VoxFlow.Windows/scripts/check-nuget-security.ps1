...
$DotnetPath = Resolve-VoxFlowDotnetPath $DotnetPath
...
function Invoke-Dotnet {
    ...
    try {
        ...
        if (-not $process.WaitForExit(1800000)) {
            ...
            throw "dotnet timed out after 1800 seconds."
        }
        $process.WaitForExit()
...
function Get-ReportedPackages {
    ...
    foreach ($project in @($Report.projects)) {
        # .NET 10 omits `frameworks` entirely for projects with no matching
        # vulnerable/deprecated packages. Under Set-StrictMode, reading the
        # missing property directly turns a clean audit into a false failure.
        ...
        foreach ($framework in @($frameworksProperty.Value)) {
            ...
            foreach ($propertyName in @("topLevelPackages", "transitivePackages")) {
                ...
                foreach ($package in @($property.Value)) {
                    ...
                    $deprecationReasonsProperty = $package.PSObject.Properties["deprecationReasons"]
                    [PSCustomObject]@{
                        ProjectPath = [string]$project.path
                        Id = [string]$package.id
                        Version = [string]$package.resolvedVersion
                        Vulnerabilities = if ($null -eq $vulnerabilitiesProperty) {
                            @()
                        }
                        else {
                            @($vulnerabilitiesProperty.Value)
                        }
                        DeprecationReasons = if ($null -eq $deprecationReasonsProperty) {
                            @()
                        }
                        else {
                            @($deprecationReasonsProperty.Value)
                        }
                    }
                }
            }
