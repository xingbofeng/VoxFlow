const release = {
  version: "1.13.0",
  tag: "v1.13.0",
  assetName: "VoxFlow-1.13.0-macOS.dmg"
};
...
const shareURL = new URL("?lang=zh&share=20260701", siteURL).toString();
const shareImageURL = assetURL("assets/wechat-share-logo.jpg?v=20260701");
...
const copy = {
  en: {
    ...
    heroImageMobileWebP: "assets/optimized/voxflow-hero-workbench-promo-en-mobile.webp",
  ...
  zh: {
    ...
    heroImageMobileWebP: "assets/optimized/voxflow-hero-workbench-promo-zh-mobile.webp",
...
const heroSourceDesktop = document.querySelector("[data-hero-source-desktop]");
const heroSourceMobile = document.querySelector("[data-hero-source-mobile]");
...
function shouldDebugWechatShare() {
  return new URLSearchParams(window.location.search).get("wechatDebug") === "1";
}
...
function setLanguage(language, { pushState = true, persist = true } = {}) {
  ...
  setMeta('link[rel="canonical"]', shareURL, "href");
  ...
  if (heroSourceDesktop) heroSourceDesktop.srcset = selected.heroImageWebP;
  if (heroSourceMobile) heroSourceMobile.srcset = selected.heroImageMobileWebP;
