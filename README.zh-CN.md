<div align="center">
  ...
  <h1>码上写 · VoxFlow</h1>
  <p><strong>语音、截图、录屏、剪切板和 coding-agent 指令的 macOS 资产工作台。</strong></p>
  <p>按 <code>⌥Space</code> 打开启动台，找回最近语音、截图、录屏和剪切板资产；按住说话、框选截图、录屏、复制内容都会沉淀为可搜索、可复制、可复用的历史资产。</p>
...
码上写是一个贴在当前应用上的资产工作台和快速启动台。它不是语音助手，不接管窗口，不自动发送内容；它把语音、截图、录屏、剪切板和 Agent 指令沉淀为可检索、可预览、可复用的本地资产，再送回你正在工作的地方。
...
| 截图/录屏记录回看 | 工作台 → 截图 | 本地截图与录屏历史和 OCR 文本 | 本地保存，可搜索、收藏、分页、复制与删除 |
...
| 页面 | 可以做什么 |
| --- | --- |
| 首页 | 查看历史资产、今日新增、来源分布和可复用内容；搜索、复制或删除语音、截图、录屏和剪切板资产 |
...
| 截图 | 浏览截图和录屏记录，查看原图和 OCR 文本，支持收藏、搜索、分页和快捷复制/删除 |
...
- **历史资产工作台**：语音 ASR 成功结果、截图、录屏、剪切板文本/图片/文件/链接/颜色统一进入资产体系，首页按资产数量、来源分布和可复用内容展示。
...
- **截图/录屏记录库**：所有截图和录屏记录（原图 + OCR 文本）都可回看，支持收藏、搜索、分页、复制与删除。
...
| 屏幕录制 | 为“任务助手”、截图 OCR 和录屏功能读取当前窗口文字与内容；截图/录屏记录保存在本机便于回看 | 系统设置 -> 隐私与安全性 -> 屏幕录制 |
...
每次 `⌘⇧A` 触发的截图或录屏都会被保留为本地记录，便于后续回看。你可以在工作台打开“截图”页面，使用关键词搜索、收藏筛选、分页浏览，并复制 OCR 文本、复制原图或删除记录。
...
- 截图/录屏记录（原图 + OCR 文本）本地保存，用于后续回看，不会上传到云端。
...
| 截图/录屏记录找不到 | 去设置 → 数据与隐私 → 数据管理检查存储状态；点击“打开数据目录”确认 `Application Support/VoxFlow/Screenshots/` 下是否有记录文件；并确认已授权屏幕录制 |
...
VoxFlow 以 GPL-3.0-or-later 分发。第三方组件仍保留各自许可证和归属说明，详见 `docs/third-party-licenses.md`。
...
扫码添加作者微信，欢迎反馈问题、交流使用体验；也可以加入码上写用户群，获取使用技巧、新版本提醒，和其他用户一起交流。
...
<table>
  <tr>
    ...
    <td align="center">
      ...
      <img src="Sources/VoxFlowApp/Resources/UserGroupQRCode.jpg" alt="码上写用户群二维码" width="300">
    </td>
  </tr>
</table>
